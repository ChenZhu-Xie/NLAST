...
def fkey(key, **kwargs):  # get_format_key
    part_z = "_z" if kwargs.get("part_z", 0) == 0 else kwargs["part_z"]
    if "_z" not in part_z:
        key = key.replace("G", "g")
    return Get("method") + ("_" + Get("way") if Get("way") != "" else "") + " - " + key + Get("ray") + part_z
...
def fGHU_plot_save(is_energy_evolution_on,  # 默认 全自动 is_auto = 1
    ...
    GHU_plot_save(fget("G"), fkey("G", **kwargs), is_energy_evolution_on,  # 这边 要省事 免代入 的话，得确保 提前 传入 ray,way,method 三个参数
                  eget("G"), is_print,
                  fget("H"), fkey("H", **kwargs),  # 以及 传入 GHU 这三个 小东西
                  fget("U"), fkey("U", **kwargs),
                  eget("U"),
                  img_name_extension,
                  # %%
                  zj, sample, size_PerPixel,
...
def fU_SSI_plot(th_f, th_e,
    ...
    if Get("is_stored") == 1:
        ...
        U_SSI_plot(sget("G"), fkey("G", **kwargs),
                   sget("U"), fkey("U", **kwargs),
                   YZget("G"), XZget("G"),
                   YZget("U"), XZget("U"),
                   get1("G"), get2("G"),
                   getf("G"), gete("G"),
...
def fU_EVV_plot(img_name_extension,
    ...
    if Get("is_stored") == 1:
        # 小写的 x,y 表示 电脑中 矩阵坐标系，大写 X,Y 表示 笛卡尔坐标系
        YZget("G")[:, Get("sheets_num")] = fget("G")[:, Get("th_X")]  # 需要 提前 fset 哦！
        XZget("G")[:, Get("sheets_num")] = fget("G")[Get("th_Y"), :]
        YZget("U")[:, Get("sheets_num")] = fget("U")[:, Get("th_X")]
        XZget("U")[:, Get("sheets_num")] = fget("U")[Get("th_Y"), :]

        # U_EVV_plot(sget("G"), fkey("G", **kwargs),
        #            sget("U"), fkey("U", **kwargs),
        #            img_name_extension,
        #            is_save_txt,
        #            # %%
...
def GU_error_energy_plot_save(G0_energy, G_energy, G_error_energy,
    ...
    args_U_error_energy_plot_save = [array_dkQ, array_Tz,
                                     img_name_extension, is_save_txt,
                                     # %%
                                     sample, size_PerPixel,
                                     is_save, dpi, size_fig_x, size_fig_y,
                                     ...
                                     # %%
                                     L0_Crystal, ]
    U_error_energy_plot_save(G0_energy, G_energy, G_error_energy, fkey("G", **kwargs),
                             *args_U_error_energy_plot_save, **kwargs, )

    U_error_energy_plot_save(U0_energy, U_energy, U_error_energy, fkey("U", **kwargs),
                             *args_U_error_energy_plot_save, **kwargs, )
