...
def H2_z_plus_dz_shift_k2_z(diz):
    return np.power(math.e, k2_z_shift * diz * 1j)  # 注意 这里的 传递函数 的 指数是 正的 ！！！


def H2_z_shift_k2_z(diz):
    return (np.power(math.e,
                     k2_z_shift * diz * 1j) - 1) / k2_z_shift ** 2 * size_PerPixel ** 2  # 注意 这里的 传递函数 的 指数是 正的 ！！！
...
def After_G2_z_plus_dz_shift_temp(for_th, fors_num, G2_z_plus_dz_shift_temp, *arg, ):
    ...
    if is_stored == 1:
        ...
        if np.mod(for_th,
            ...
            G2_z_shift_stored[:, :,
            int(for_th // (sheets_num // sheets_stored_num))] = G2_z_plus_dz_shift_temp  # 储存的 第一层，实际上不是 G2_0，而是 G2_dz
            U2_z_stored[:, :,
...
if is_save == 1:
    ...
    np.savetxt(G2_z0_SSI_shift_txt_address, G2_z0_SSI_shift) if is_save_txt else savemat(G2_z0_SSI_shift_txt_address,
                                                                                         {'G': G2_z0_SSI_shift})
...
if is_save == 1:
    ...
    np.savetxt(H2_z0_SSI_shift_txt_address, H2_z0_SSI_shift) if is_save_txt else savemat(H2_z0_SSI_shift_txt_address,
                                                                                         {"H": H2_z0_SSI_shift})
...
if is_stored == 1:
    ...
    if is_show_structure_face == 1:
        G2_structure_frontface_shift_amp_address = location + "\\" + "5. G2_" + str(
            float('%.2g' % z0)) + "mm" + "_SSI_shift" + "_sheets_selective_stored" + "\\" + "5.1. NLA - " + "G2_" + str(
        ...
        G2_structure_endface_shift_amp_address = location + "\\" + "5. G2_" + str(
            float('%.2g' % z0)) + "mm" + "_SSI_shift" + "_sheets_selective_stored" + "\\" + "5.1. NLA - " + "G2_" + str(
    ...
    if is_show_structure_face == 1:
        G2_structure_frontface_shift_phase_address = location + "\\" + "5. G2_" + str(
            float('%.2g' % z0)) + "mm" + "_SSI_shift" + "_sheets_selective_stored" + "\\" + "5.2. NLA - " + "G2_" + str(
        ...
        G2_structure_endface_shift_phase_address = location + "\\" + "5. G2_" + str(
            float('%.2g' % z0)) + "mm" + "_SSI_shift" + "_sheets_selective_stored" + "\\" + "5.2. NLA - " + "G2_" + str(
    ...
    if is_show_structure_face == 1:
        vmax_U2_section_1_2_front_end_shift_phase = np.max(
            [np.max(np.angle(U2_section_1)), np.max(np.angle(U2_section_2)), np.max(np.angle(U2_structure_frontface)),
             np.max(np.angle(U2_structure_endface))])
        vmin_U2_section_1_2_front_end_shift_phase = np.min(
            [np.min(np.angle(U2_section_1)), np.min(np.angle(U2_section_2)), np.min(np.angle(U2_structure_frontface)),
             np.min(np.angle(U2_structure_endface))])
    else:
        vmax_U2_section_1_2_front_end_shift_phase = np.max(
