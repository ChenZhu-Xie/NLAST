...
if __name__ == '__main__':
    kwargs = \
        {"U_name": "",
         ...
         # %%
         "theta_z": 0.5, "phi_z": 0, "phi_c": 0,  # "phi_z": 180 即 双轴 2
    ...
    from fun_linear import Gan_refractive_index_ellipsoid
    import numpy as np
    nx, ny, nz = Gan_refractive_index_ellipsoid(kwargs["is_air"], kwargs["lam1"], kwargs["T"])
    tan_Omega = nz / nx * ((ny ** 2 - nx ** 2)/(nz ** 2 - ny ** 2)) ** 0.5
    Omega = np.arctan(tan_Omega) / np.pi * 180
    print(Omega)
    kwargs["theta_z"] += Omega  # 本来是 = Omega 即覆盖，但这里 改为了 增量，为的是 微调 Omega，看看效果。
