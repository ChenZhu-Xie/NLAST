        ...
        if is_mm == 1:  # round(i * size_PerPixel,2) 保留 2 位小数，改为 保留 2 位 有效数字
            if is_propagation != 0:
                ...
                axes.set_xticklabels(
                    [float('%.2g' % (i * size_PerPixel)) for i in range(- Iz // 2, Iz - Iz // 2, Iz // ticks_num)],
                    fontsize=fontsize, fontdict=font)
...
def plot_3d_XYZ(Iz=700, Iy=343, Ix=343, size_PerPixel=0.007, diz=0.774,
    ...
    if is_self_colorbar == 1:
        ...
        i_XY = i_XY[::-1]
        img = axes.scatter3D(i_Z, iX, i_XY, c=U_YZ if is_energy != 1 else U_YZ ** 2, cmap=cmap_3d,
                             alpha=math.e ** (-1 * alpha))
        i_XY = i_XY[::-1]
        img = axes.scatter3D(i_Z, i_XY, iY, c=U_XZ if is_energy != 1 else U_XZ ** 2, cmap=cmap_3d,
                             alpha=math.e ** (-1 * alpha))

        i_X, i_Y = np.meshgrid([i for i in range(Ix)], [j for j in range(Iy)])
        i_Y = i_Y[::-1]
        img = axes.scatter3D(iZ_1, i_X, i_Y, c=U_1 if is_energy != 1 else U_1 ** 2, cmap=cmap_3d,
                             alpha=math.e ** (-1 * alpha))
        img = axes.scatter3D(iZ_2, i_X, i_Y, c=U_2 if is_energy != 1 else U_2 ** 2, cmap=cmap_3d,
                             alpha=math.e ** (-1 * alpha))

        if is_show_structure_face == 1:
            img = axes.scatter3D(iZ_structure_front, i_X, i_Y,
                                 c=U_structure_front if is_energy != 1 else U_structure_front ** 2, cmap=cmap_3d,
                                 alpha=math.e ** (-1 * alpha))
            img = axes.scatter3D(iZ_structure_end, i_X, i_Y,
...
def plot_3d_XYz(Iy=343, Ix=343, size_PerPixel=0.007, diz=0.774,
    ...
    if is_self_colorbar == 1:
        ...
        for sheet_stored_th in range(sheets_stored_num + 1):
            img = axes.scatter3D(sheet_th_stored[sheet_stored_th], i_X, i_Y,
