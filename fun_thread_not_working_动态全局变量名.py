...
def my_thread(threads_num, fors_num, 
    ...
    names = globals()
    print(names)
    names['thread_th_' + str(args[0])] = 0
    names['for_th_' + str(args[0])] = 0
    ...
    class Producer(threading.Thread):
        ...
        def run(self):
            ...
            while True:
                ...
                if self.for_th >= self.fors_num and names['for_th_' + str(args[0])] == self.fors_num: # 退出生产线程 的 条件：p 线程 完成 其母线程功能，且 最后一个 t 线程 完成其子线程功能
                    break
                else:
                    if names['thread_th_' + str(args[0])] >= self.threads_num or self.for_th == self.fors_num : # 暂停生产线程 的 条件： 运行线程数 达到 设定，或 p 线程 完成 其母线程功能
                        con.notify()
                        con.wait()
                    else:
                        # print(self.for_th)
                        t = Customer('thread:%s' % names['thread_th_' + str(args[0])], self.for_th)
                        t.setDaemon(True)
                        t.start()
                        
                        names['thread_th_' + str(args[0])] += 1
    ...
    class Customer(threading.Thread):
        ...
        def run(self):
            ...
            if is_ordered == 1:
                ...
                while True:
                    if names['for_th_' + str(args[0])] == self.for_th:
                        # print(self.for_th)
                        """----- your code begin 2 -----"""
            ...
            names['thread_th_' + str(args[0])] -= 1 # 在解锁之前 减少 1 个线程数量，以便 p 线程 收到消息后，生产 1 个 线程出来
