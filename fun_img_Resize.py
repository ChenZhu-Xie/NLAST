...
def image_Add_black_border(img_full_name = "Grating.png", 
    ...
    if img.shape[0] != img.shape[1]: # 如果 高 ≠ 宽
        if img.shape[0] < img.shape[1]:# 如果 图片很宽，就上下加 黑色_不透明_边框
            image_Border(img_address, img_squared_address, loc='t', width=( img.shape[1] - img.shape[0] ) // 2, color=(0, 0, 0, 255))
            if ( img.shape[1] - img.shape[0] ) % 2 == 1: # 如果 宽高差 是 奇数，则 下边框 多加一个 像素
                image_Border(img_squared_address, img_squared_address, loc='b', width=( img.shape[1] - img.shape[0] ) // 2 + 1, color=(0, 0, 0, 255))
            else:
                image_Border(img_squared_address, img_squared_address, loc='b', width=( img.shape[1] - img.shape[0] ) // 2, color=(0, 0, 0, 255))
        else: # 如果 图片很高，就左右加 黑色_不透明_边框
            image_Border(img_address, img_squared_address, loc='l', width=( img.shape[0] - img.shape[1] ) // 2, color=(0, 0, 0, 255))
            if ( img.shape[0] - img.shape[1] ) % 2 == 1: # 如果 高宽差 是 奇数，则 右边框 多加一个 像素
                image_Border(img_squared_address, img_squared_address, loc='r', width=( img.shape[0] - img.shape[1] ) // 2 + 1, color=(0, 0, 0, 255))
            else:
                image_Border(img_squared_address, img_squared_address, loc='r', width=( img.shape[0] - img.shape[1] ) // 2, color=(0, 0, 0, 255))
    else:
...
def img_squared_Resize(img_name, img_name_extension, img_squared, 
    ...
    is_print and print("structure_squared.shape = img_squared_resize_bordered.shape = {}".format(img_squared_resize_bordered.shape))
